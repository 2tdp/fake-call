...
public class BitmapLoadTask extends AsyncTask<Void, Void, BitmapLoadTask.BitmapWorkerResult> {
    ...
    protected BitmapWorkerResult doInBackground(Void... params) {
        ...
        if (exifDegrees != 0) matrix.preRotate(exifDegrees);
        if (exifTranslation != 1) matrix.postScale(exifTranslation, 1);
    ...
    private void downloadFile(@NonNull Uri inputUri, @Nullable Uri outputUri) throws NullPointerException, IOException {
        ...
        try {
            ...
            if (outputStream != null) {
                ...
                source.readAll(sink);
            } else throw new NullPointerException("OutputStream for given output Uri is null");
        } finally {
            BitmapLoadUtils.close(source);
            BitmapLoadUtils.close(sink);
            if (response != null) BitmapLoadUtils.close(response.body());
    ...
    protected void onPostExecute(@NonNull BitmapWorkerResult result) {
        ...
        else mBitmapLoadCallback.onFailure(result.mBitmapWorkerException);
    }
