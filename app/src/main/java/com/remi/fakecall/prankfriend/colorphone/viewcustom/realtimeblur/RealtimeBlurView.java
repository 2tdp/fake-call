package com.remi.fakecall.prankfriend.colorphone.viewcustom.realtimeblur;
...
public class RealtimeBlurView extends View {
	...
	protected BlurImpl getBlurImpl() {
		...
		if (BLUR_IMPL == 0) {
			try {
				Objects.requireNonNull(getClass().getClassLoader()).loadClass("androidx.renderscript.RenderScript");
		...
		if (BLUR_IMPL == 0) {
			try {
				Objects.requireNonNull(getClass().getClassLoader()).loadClass("android.support.v8.renderscript.RenderScript");
				// initialize RenderScript to load jni impl
				// may throw unsatisfied link error
		...
		return switch (BLUR_IMPL) {
			case 1 -> new AndroidXBlurImpl();
			case 2 -> new SupportLibraryBlurImpl();
			case 3 -> new AndroidStockBlurImpl();
			default -> new EmptyBlurImpl();
		};
	}
	...
	protected boolean prepare() {
		...
		if (dirty) {
			if (mBlurImpl.prepare(getContext(), mBitmapToBlur, radius)) mDirty = false;
			else return false;
		}
		...
		public boolean onPreDraw() {
			...
			if (decor != null && isShown() && prepare()) {
				...
				try {
					...
					if (decor.getBackground() != null) decor.getBackground().draw(mBlurringCanvas);
	...
	protected View getActivityDecorView() {
		...
		if (ctx instanceof Activity) return ((Activity) ctx).getWindow().getDecorView();
		else return null;
	}
	...
	protected void onAttachedToWindow() {
		...
		if (mDecorView != null) {
			...
			if (mDifferentRoot) mDecorView.postInvalidate();
		} else mDifferentRoot = false;
	}
