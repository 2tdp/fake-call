...
public class DateHelper {
    ...
    public TimeZone getTimeZone() {
        if(this.timeZone == null) return TimeZone.getDefault();
        else return timeZone;
    }

    public Calendar getCalendarOfDate(Date date) {
        final Calendar calendar = Calendar.getInstance(getTimeZone(), Locale.ENGLISH);
    ...
    public int getHour(Date date, boolean isAmPm) {
        if (isAmPm) return getHourOfDay(date);
        else return getHour(date);
    }
