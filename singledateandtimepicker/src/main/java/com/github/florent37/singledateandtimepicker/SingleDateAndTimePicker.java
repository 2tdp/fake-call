...
public class SingleDateAndTimePicker extends LinearLayout {
    ...
    public SingleDateAndTimePicker(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        for (WheelPicker wheelPicker : pickers) wheelPicker.setDateHelper(dateHelper);
    ...
    protected void onAttachedToWindow() {
        ...
        yearsPicker.setOnYearSelectedListener((picker, position, year) -> {
            ...
            if (displayDaysOfMonth) updateDaysOfMonth();
        });
        ...
        daysOfMonthPicker.setDayOfMonthSelectedListener((picker, dayIndex) -> {
                    updateListener();
                    checkMinMaxDate(picker);
                });

        daysOfMonthPicker.setOnFinishedLoopListener(picker -> {
                    if (displayMonth) {
        ...
        minutesPicker.setOnMinuteChangedListener((picker, minutes) -> {
                    updateListener();
                    checkMinMaxDate(picker);
                })
                .setOnFinishedLoopListener(picker -> hoursPicker.scrollTo(hoursPicker.getCurrentItemPosition() + 1));

        hoursPicker.setOnFinishedLoopListener(picker -> daysPicker.scrollTo(daysPicker.getCurrentItemPosition() + 1))
                .setHourChangedListener((picker, hour) -> {
                    updateListener();
                    checkMinMaxDate(picker);
                });

        amPmPicker.setAmPmListener((picker, isAm) -> {
                    updateListener();
                    checkMinMaxDate(picker);
                });
    ...
    public void setEnabled(boolean enabled) {
        ...
        for (WheelPicker picker : pickers) picker.setEnabled(enabled);
    }
    ...
    public void setItemSpacing(int size) {
        for (WheelPicker picker : pickers) picker.setItemSpace(size);
    }

    public void setCurvedMaxAngle(int angle) {
        for (WheelPicker picker : pickers) picker.setCurvedMaxAngle(angle);
    }

    public void setCurved(boolean curved) {
        for (WheelPicker picker : pickers) picker.setCurved(curved);
    }

    public void setCyclic(boolean cyclic) {
        for (WheelPicker picker : pickers) picker.setCyclic(cyclic);
    }

    public void setTextSize(int textSize) {
        for (WheelPicker picker : pickers) picker.setItemTextSize(textSize);
    }
    ...
    public void setTextColor(int textColor) {
        for (WheelPicker picker : pickers) picker.setItemTextColor(textColor);
    }

    public void setTextAlign(int align) {
        for (WheelPicker picker : pickers) picker.setItemAlign(align);
    }

    public void setTypeface(Typeface typeface) {
        if(typeface == null) return;
        for (WheelPicker picker : pickers) picker.setTypeface(typeface);
    }
    ...
    public void setDayFormatter(SimpleDateFormat simpleDateFormat) {
        if (simpleDateFormat != null) this.daysPicker.setDayFormatter(simpleDateFormat);
    }
    ...
    private void checkBeforeMinDate(final WheelPicker picker) {
        picker.postDelayed(() -> {
            if (minDate != null && isBeforeMinDate(getDate()))
                for (WheelPicker p : pickers) p.scrollTo(p.findIndexOfDate(minDate));
    ...
    private void checkAfterMaxDate(final WheelPicker picker) {
        picker.postDelayed(() -> {
            if (maxDate != null && isAfterMaxDate(getDate()))
                for (WheelPicker p : pickers) p.scrollTo(p.findIndexOfDate(maxDate));
        }, DELAY_BEFORE_CHECK_PAST);
    }
    ...
    public void checkPickersMinMax() {
        for (WheelPicker picker : pickers) checkMinMaxDate(picker);
    }

    public Date getDate() {
        int hour = hoursPicker.getCurrentHour();
        if (isAmPm && amPmPicker.isPm()) hour += PM_HOUR_ADDITION;
        ...
        if (displayDays) {
            ...
            if (displayMonth) calendar.set(Calendar.MONTH, monthPicker.getCurrentMonth());
            if (displayYears) calendar.set(Calendar.YEAR, yearsPicker.getCurrentYear());

            if (displayDaysOfMonth) {
                int daysInMonth = calendar.getActualMaximum(Calendar.DAY_OF_MONTH);
                if (daysOfMonthPicker.getCurrentDay() >= daysInMonth)
                    calendar.set(Calendar.DAY_OF_MONTH, daysInMonth);
                else calendar.set(Calendar.DAY_OF_MONTH, daysOfMonthPicker.getCurrentDay() + 1);
            }
        }
    ...
    public void setDefaultDate(Date date) {
        if (date != null) {
            ...
            for (WheelPicker picker : pickers) picker.setDefaultDate(defaultDate);
        }
    }
    ...
    public void selectDate(Calendar calendar) {
        ...
        for (WheelPicker picker : pickers) picker.selectDate(date);
    ...
    private void updateListener() {
        ...
        for (OnDateChangedListener listener : listeners) listener.onDateChanged(displayed, date);
    }
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        setTypeface(Typeface.createFromAsset(context.getAssets(), "fonts/roboto/roboto_regular.ttf"));
