...
public abstract class WheelPicker<V> extends View {
    ...
    private int measureSize(int mode, int sizeExpect, int sizeActual) {
        int realSize;
        if (mode == MeasureSpec.EXACTLY) realSize = sizeExpect;
        else {
            realSize = sizeActual;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int drawnDataPos = drawnDataStartPos + selectedItemPosition,
            ...
            if (isCurved) {
                ...
                float ratio = (drawnCenterY - Math.abs(drawnCenterY - mDrawnItemCenterY) - rectDrawn.top) * 1.0F / (drawnCenterY - rectDrawn.top);

                // Correct unit
                int unit = 0;
                if (mDrawnItemCenterY > drawnCenterY) unit = 1;
            ...
            switch (event.getAction()) {
                ...
                case MotionEvent.ACTION_UP:
                    ...
                    if (Math.abs(velocity) > minimumVelocity) {
                        ...
                        scroller.setFinalY(scroller.getFinalY() + computeDistanceToEndPoint(scroller.getFinalY() % mItemHeight));
                    } else
                        scroller.startScroll(0, scrollOffsetY, 0, computeDistanceToEndPoint(scrollOffsetY % mItemHeight));

                    // Correct coordinates
                    if (!isCyclic) {
                        if (scroller.getFinalY() > maxFlingY) scroller.setFinalY(maxFlingY);
    ...
    private int computeDistanceToEndPoint(int remainder) {
        if (Math.abs(remainder) > mHalfItemHeight) {
            if (scrollOffsetY < 0) return -mItemHeight - remainder;
    ...
    protected void onItemCurrentScroll(int position, V item) {
        if (lastScrollPosition != position) {
            if (listener != null) {
                ...
                if (lastScrollPosition == adapter.getItemCount() - 1 && position == 0) onFinishedLoop();
            }
    ...
    public int getTodayItemPosition() {
        ...
        for (int i = 0; i < list.size(); i++) {
            if (list.get(i) instanceof DateWithLabel) {
                ...
                if (dwl.label.equals(getLocalizedString(R.string.picker_today))) return i;
            }
        }
        return 0;
    }
    ...
    public void notifyDatasetChanged() {
        ...
        else selectedItemPosition = currentItemPosition;
    ...
    public int findIndexOfDate(@NonNull Date date) {
        ...
        if (this instanceof WheelDayPicker) {
            ...
            if (today.equals(formatItem)) return getTodayItemPosition();
        }
        ...
        for (int i = 0; i < itemCount; ++i) {
            ...
            if (formatItemInt != Integer.MIN_VALUE) {
                ...
                if (objectInt <= formatItemInt) index = i;
            } else if (formatItem.equals(object)) return i;
        }
        return index;
    }
    ...
    public static class Adapter<V> implements BaseAdapter {
        ...
        public int getItemPosition(V value) {
            int position = -1;
            if (data != null) return data.indexOf(value);
            return position;
        }
    }
}
